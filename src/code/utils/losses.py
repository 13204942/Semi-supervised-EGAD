...
def hd_loss(preds, target):
    preds = F.one_hot(preds.argmax(dim=1).long(), 3).permute(0,3,1,2)  # (BS,3,224,224)
    target = F.one_hot(target.long(), 3).permute(0,3,1,2)  # (BS,3,224,224)   
    assert preds.size() == target.size()    
    hd_distance = HausdorffDistance(distance_metric="euclidean", num_classes=3)
    loss = hd_distance(preds, target)
    return loss
...
def info_nce_loss_orig(feats1,feats2):
    # Encode all images
    # Calculate cosine similarity
    cos_sim = F.cosine_similarity(feats1[:,None,:], feats2[None,:,:], dim=-1)
    # Mask out cosine similarity to itself
