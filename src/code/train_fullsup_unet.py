...
from utils.losses import ConstraLoss, info_nce_loss
...
def train(args, snapshot_path):
    ...
    model1 = Unet2D(in_chns=3, class_num=args.num_classes).to(device)
    ...
    db_train_labled = BaseDataSets_HC18(base_dir=args.root_path, split="suptrain", num=None, islabeled=True, 
    ...
    db_val = BaseDataSets_HC18(base_dir=args.root_path, split="val", transform=None, default_transform=tensor_transforms)
    ...
    trainloader = DataLoader(db_train, batch_size=batch_size, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print(f'volume_batch max: {volume_batch.max()}')
            # print(f'volume_batch min: {volume_batch.min()}')
            # print(f'label_batch max: {label_batch.max()}')
            # print(f'label_batch min: {label_batch.min()}')  
            # print(f'volume_batch unique:{torch.unique(volume_batch)}')
            
            outputs1 = model1(volume_batch)
            outputs1_emb = model1.encoder(volume_batch)[-1]
            outputs_soft1 = torch.softmax(outputs1, dim=1)

            # print(f'outputs1 shape: {outputs1.shape}') # B, 3, 224, 224
            # print(f'outputs1_emb shape: {outputs1_emb.shape}') # B, 256, 14, 14
            # print(f'outputs2 shape: {outputs1.shape}') # B, 3, 224, 224
            # print(f'outputs2_emb shape: {outputs2_emb.shape}') # B, 49, 768
            # print(f'label_batch shape: {label_batch.shape}') # B, 1, 224, 224
            
            loss1 = ce_loss(outputs1[:args.labeled_bs], torch.squeeze(label_batch[:args.labeled_bs],1).long()) + \
            ...
            writer.add_scalar('loss/model1_loss', loss1, iter_num)
            logging.info('iteration %d : model1 loss : %f' % (iter_num, loss.item()))
            ...
            if iter_num > 0 and iter_num % 500 == 0:
                ...
                if performance1 > best_performance1 and iter_num > 15000:
            ...
            if iter_num % 1000 == 0:
                ...
                cv2.imwrite(f"{args.root_path}/ssl/assets_fullsup/image_{iter_num}.png",np.transpose(image_save.data.cpu().numpy(),(1,2,0))) 
                ...
                cv2.imwrite(f"{args.root_path}/ssl/assets_fullsup/outputs1_{iter_num}.png",img1_outputs.data.cpu().numpy())
                ...
                cv2.imwrite(f"{args.root_path}/ssl/assets_fullsup/labs_{iter_num}.png",np.transpose(labs_tensor.data.cpu().numpy(),(1,2,0)))
...
if __name__ == "__main__":
    ...
    snapshot_path = "/mnt/storage/fangyijie/HC18/ssl/model/{}_{}/{}".format(
