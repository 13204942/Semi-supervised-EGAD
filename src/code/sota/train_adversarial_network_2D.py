...
def train(args, snapshot_path):
    ...
    def create_model(ema=False):
        # Network definition
        model = net_factory(net_type=args.model, in_chns=3, class_num=num_classes).to(device)
        if ema:
    ...
    DAN = FCDiscriminator(num_classes=num_classes, ndf=32, n_channel=3)
    DAN = DAN.to(device)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            DAN_target = torch.tensor([0] * args.batch_size).to(device)
            ...
            # print(outputs_soft[args.labeled_bs:].shape)
            DAN_outputs = DAN(outputs_soft[args.labeled_bs:], volume_batch[args.labeled_bs:])

            # print(DAN_outputs[:,1].shape)
            # print(DAN_target[args.labeled_bs:].shape)
            consistency_loss = F.cross_entropy(DAN_outputs[:,1], DAN_target[args.labeled_bs:].to(torch.float))
            ...
            if iter_num % 250 == 0:
                ...
                cv2.imwrite(f"{args.root_path}/ssl/assets_DAN_unext_{labeled_slice}/image_{iter_num}.png",np.transpose(image_save.data.cpu().numpy(),(1,2,0))) 
                writer.add_image('train/Image', image_save, iter_num)

                outputs = torch.argmax(torch.softmax(outputs, dim=1), dim=1, keepdim=True)
                img1_outputs = np.squeeze(outputs[0]) * 100
                cv2.imwrite(f"{args.root_path}/ssl/assets_DAN_unext_{labeled_slice}/outputs1_{iter_num}.png",img1_outputs.data.cpu().numpy())
                writer.add_image('train/Prediction', outputs[0, ...] * 100, iter_num, dataformats='CHW')

                labs_tensor = label_batch[0, ...] * 100
                labs = label_batch[1, ...].unsqueeze(0) * 50
                cv2.imwrite(f"{args.root_path}/ssl/assets_DAN_unet_{labeled_slice}/labs_{iter_num}.png",np.transpose(labs_tensor.data.cpu().numpy(),(1,2,0)))
